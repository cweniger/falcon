...
class Flow(torch.nn.Module):
    def __init__(self, theta, s, log_prefix=None, **kwargs):
        super(Flow, self).__init__()
        self.log_prefix = log_prefix + ":" if log_prefix else ""
        self.net = net_builders.build_nsf(theta, s, z_score_x=None, z_score_y=None)
        #self.net = net_builders.build_zuko_gf(theta, s, z_score_x=None, z_score_y=None)
        # Learnable logvar
        self.logvar = torch.nn.Parameter(torch.zeros(theta.shape[1]))
        # Estimation of mean value of theta
        self.mean_estimator_MLP = torch.nn.Sequential(
            torch.nn.Linear(s.shape[1], 128),
            torch.nn.ReLU(),
            torch.nn.Linear(128, 128),
            torch.nn.ReLU(),
            torch.nn.Linear(128, theta.shape[1])
        )
        self.alpha = 0  # 0: detach, 1: no detach

    def loss(self, theta, s):
        normalized_theta = (theta - self.mean_estimator_MLP(s)) / torch.exp(0.5*self.logvar).unsqueeze(0)
        normalized_theta = normalized_theta.float()
        loss1 = 0.5 * (self.logvar.sum() + (normalized_theta**2).sum(dim=1))
        loss2 = self.net.loss(self.alpha*normalized_theta + (1-self.alpha)*normalized_theta.detach(), condition=s)  # Flow
        # second loss contribution for mean and logvar, based on Gaussian log prob
    ...
    def sample(self, num_samples, s):
        ...
        mean = self.mean_estimator_MLP(s)
        samples = samples*torch.exp(0.5*self.logvar).unsqueeze(0) + mean
        return samples
    ...
    def log_prob(self, theta, s):
        ...
        normalized_theta = (theta - self.mean_estimator_MLP(s)) / torch.exp(0.5*self.logvar).unsqueeze(0)
        normalized_theta = normalized_theta.float()
        log_prob = self.net.log_prob(normalized_theta, condition=s)  # (num_proposals, num_samples)
        # Volume correction
        log_prob = log_prob - 0.5*self.logvar.sum()
        return log_prob
...
class SNPE_A_clean:
    ...
    def proposal_sample(self, num_samples, parent_conditions=[], evidence_conditions=[]):
        ...
        vector_min = samples.min(axis=0).values.cpu()
        vector_max = samples.max(axis=0).values.cpu()
        vector_std = samples.std(axis=0).cpu()
        log(
            {f"proposal_mean_{i}": vector_mean[i].item() for i in range(len(vector_mean))},
        )
        log(
            {f"proposal_min_{i}": vector_min[i].item() for i in range(len(vector_min))},
        )
        log(
            {f"proposal_max_{i}": vector_max[i].item() for i in range(len(vector_max))},
        )
        log(
    ...
    def _aux_sample(self, num_samples, mode = None, parent_conditions=[], evidence_conditions=[]):
        ...
        if proposal_mode == 'traindist':
            ...
            traindist_net.eval()
            posterior_net.eval()
            samples_proposals1 = traindist_net.sample(num_proposals//2, s*0).detach()
            samples_proposals2 = posterior_net.sample(num_proposals//2, s).detach()
            samples_proposals = torch.cat([samples_proposals1, samples_proposals2], dim=0)
            log_prob_dist1 = traindist_net.log_prob(
                samples_proposals, s*0)  # (num_proposals, num_samples)
            log_prob_dist2 = posterior_net.log_prob(
                samples_proposals, s)  # (num_proposals, num_samples)
            log_prob_dist = torch.log(torch.exp(log_prob_dist1)*0.5 + torch.exp(log_prob_dist2)*0.5)
        else:
            traindist_net.eval()
            samples_proposals = traindist_net.sample(num_proposals, s*0).detach()
            log_prob_dist = traindist_net.log_prob(
        ...
        log({"mask rejection_fraction": mask.float().mean().item()})
        mask = mask.any(dim=-1).float()*10000     # (num_proposals, num_samples)
        ...
        log_weights = torch.nan_to_num(log_weights, nan=-10000.0, neginf=-10000.0)
