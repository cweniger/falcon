#!/usr/bin/env python3
"""
Linear regression posterior learning with simulation buffer and sample reuse.
Based on gaussian_lr3.py, adds a fixed-size simulation buffer with configurable
replacement fraction to study the effect of training data renewal.
...
Buffer mechanics:
  ...
  - Then replace the oldest replacement_fraction * batch_size entries
    with fresh proposal samples
  - replacement_fraction=1: full turnover every buffer_size/batch_size steps
  - replacement_fraction=1/batch_size: one sample replaced per step
...
def main():
    ...
    # Print header
    print(f"{'Step':>6} {'SimTot':>10} {'ValLoss':>12} {'mean_var_r':>12} {'mean_std_r':>12}")
    print("-" * 56)
    ...
    # Sequential phase: sample from buffer, replace oldest with fresh proposals
    for step in range(cfg.num_steps):
        ...
        # 3. Replace oldest samples with fresh proposals
        ...
        if n_replace >= 1:
            ...
            z_new = sample_proposal(model, y_obs, n_replace, cfg.gamma, device, step, cfg.prior_only)
        ...
        if step % cfg.print_every == 0 or step == cfg.num_steps - 1:
            with torch.no_grad():
                # Evaluate on fresh validation samples from the proposal
                z_val = sample_proposal(model, y_obs, cfg.batch_size, cfg.gamma, device, step, cfg.prior_only)
            ...
            print(f"{step:>6} {total_sims:>10} {val_loss:>12.2e} {mean_var_ratio:>12.3f} {mean_std_ratio:>12.3f}")
