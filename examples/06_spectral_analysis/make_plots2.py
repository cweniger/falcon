...
For config2.yml (SVD scaffolding variant): signal params are on the y node,
noise sigma on the n node.

Usage: python make_plots2.py [RUN_PATH]    (default: outputs/latest)
"""
...
def main():
    ...
    # ── Signal parameters from config (config2 graph structure) ─────
    sig_cfg = run.config.graph.y.simulator
    ...
    noise_cfg = run.config.graph.n.simulator
    sigma = float(noise_cfg.get("sigma", 1.0))
    ...
    # ── Corner plot ─────────────────────────────────────────────────
    # Use 5-sigma CRB range centered on truth to avoid degenerate axes
    plot_range = [
        (true_theta[i] - 5 * std_crb[i], true_theta[i] + 5 * std_crb[i])
