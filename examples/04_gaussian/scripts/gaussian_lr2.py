...
Diagonal normalization version (no Cholesky; input/output whitened by std only).
...
class NormalizedMLP(nn.Module):
    """MLP with diagonal online normalization and eigenvalue-based covariance tracking.

    Input/output normalization uses per-element mean and std (diagonal),
    instead of full Cholesky whitening.
    """
    ...
    def __init__(self, obs_dim, hidden_dim=128, num_layers=3, activation='relu',
        ...
        # Diagonal normalization: mean and std vectors
...
def main():
    ...
    print(f"Using diagonal normalization with eigenvalue-based tempered likelihood proposal")
