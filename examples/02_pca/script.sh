...
for net_type in nice; do
    ...
    falcon swarm wandb.project=falcon_tmp3 wandb.group=zuko_${net_type}_v1 graph.z.simulate.net_type=zuko_${net_type} \
	    directories.sim_dir=/scratch-local/cweniger/sim_dir \
	    directories.graph_dir=/scratch-local/cweniger/graph_dir
    falcon simulate wandb.project=falcon_tmp3 wandb.group=zuko_${net_type}_v1 graph.z.simulate.net_type=zuko_${net_type} \
	    directories.sim_dir=/scratch-local/cweniger/sim_dir \
	    directories.graph_dir=/scratch-local/cweniger/graph_dir
done
