...
class TestInitialSamplesLoading:
    ...
    def _create_prior_samples(self, tmp_path, n_samples=5):
        """Create a realistic prior sample directory structure."""
        prior_dir = tmp_path / "prior"
        batch_dir = prior_dir / "260218-1200"
        batch_dir.mkdir(parents=True)

        for i in range(n_samples):
            np.savez(
                batch_dir / f"{i:06d}.npz",
                z=np.random.randn(3),
    ...
    def test_keys_are_remapped(self, tmp_path):
        ...
        prior_dir = tmp_path / "prior" / "260218-1200"
        prior_dir.mkdir(parents=True)
        np.savez(prior_dir / "000000.npz", z=np.array([1.0, 2.0]), x=np.array([0.5]))

        samples = _load_initial_samples(tmp_path / "prior")
        sample = samples[0]
    ...
    def test_metadata_keys_skipped(self, tmp_path):
        ...
        assert "_batch" not in sample
        assert "_batch.value" not in sample
    ...
    def test_multiple_batches(self, tmp_path):
        """Samples from multiple batch directories are all loaded."""
        prior_dir = tmp_path / "prior"
        batch1 = prior_dir / "260218-1200"
        batch2 = prior_dir / "260218-1400"
        ...
        # All should have the correct keys
        for s in samples:
            assert "z.value" in s
            assert "z.log_prob" in s
