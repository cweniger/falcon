...
EXAMPLE_CONFIGS = [
    ...
    ("01_minimal", "config.yml", ["graph.z.estimator.max_epochs=2"]),
    # 02_bimodal: single estimator 'z', using config_regular (needs GPU override)
    ("02_bimodal", "config_regular.yml", ["graph.z.estimator.max_epochs=2", "graph.z.ray.num_gpus=0"]),
    # 03_composite: two ResNet18 + Ray actors exceed CI runner memory
    pytest.param(
        "03_composite", "config.yml",
        ["graph.z1.estimator.max_epochs=2", "graph.z2.estimator.max_epochs=2",
    ...
    ("04_gaussian", "config.yml", ["graph.z.estimator.max_epochs=2", "graph.z.ray.num_gpus=0"]),
    # 05_linear_regression: requires GPU
    pytest.param(
        "05_linear_regression", "config.yml",
        ["graph.theta.estimator.max_epochs=2", "graph.theta.ray.num_gpus=0"],
        marks=_skip_ci,
    ),
]
